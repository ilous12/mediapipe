...
#include "mediapipe/calculators/image/mask_blend_calculator.pb.h"
...
// A calculator to mask blend a masked area of an image to a specified color.
//
...
//    node_options: {
//      [mediapipe.MaskBlendCalculatorOptions] {
//        color { r: 0 g: 0 b: 255 }
//        mask_channel: RED
//      }
//    }
...
class MaskBlendCalculator : public CalculatorBase {
 public:
  MaskBlendCalculator() = default;
  ~MaskBlendCalculator() override = default;
 ...
 private:
  ...
  mediapipe::MaskBlendCalculatorOptions::MaskChannel mask_channel_;
...
REGISTER_CALCULATOR(MaskBlendCalculator);

// static
absl::Status MaskBlendCalculator::GetContract(CalculatorContract* cc) {
...
absl::Status MaskBlendCalculator::Open(CalculatorContext* cc) {
...
absl::Status MaskBlendCalculator::Process(CalculatorContext* cc) {
  if (use_gpu_) {
#if !MEDIAPIPE_DISABLE_GPU
    MP_RETURN_IF_ERROR(
...
absl::Status MaskBlendCalculator::Close(CalculatorContext* cc) {
#if !MEDIAPIPE_DISABLE_GPU
...
absl::Status MaskBlendCalculator::RenderCpu(CalculatorContext* cc) {
  ...
  if (mask_mat.channels() > 1) {
    ...
    if (mask_channel_ == mediapipe::MaskBlendCalculatorOptions_MaskChannel_ALPHA)
      mask_mat = channels[3];
    else
      mask_mat = channels[0];
  }
...
absl::Status MaskBlendCalculator::RenderGpu(CalculatorContext* cc) {
...
void MaskBlendCalculator::GlRender() {
#if !MEDIAPIPE_DISABLE_GPU
...
absl::Status MaskBlendCalculator::LoadOptions(CalculatorContext* cc) {
  const auto& options = cc->Options<mediapipe::MaskBlendCalculatorOptions>();
...
absl::Status MaskBlendCalculator::InitGpu(CalculatorContext* cc) {
#if !MEDIAPIPE_DISABLE_GPU
  ...
  switch (mask_channel_) {
    case mediapipe::MaskBlendCalculatorOptions_MaskChannel_UNKNOWN:
    case mediapipe::MaskBlendCalculatorOptions_MaskChannel_RED:
      mask_component = "r";
      break;
    case mediapipe::MaskBlendCalculatorOptions_MaskChannel_ALPHA:
      mask_component = "a";
      break;
  }
  ...
  const std::string frag_src = R"(
    ...
    void main() {
      ...
      fragColor = vec4(weight.MASK_COMPONENT, 1.0, 1.0, 1.0);
    }
  )";
